...
- **Grid** opens a spatial autoplay carousel containing only the current face,
  globe, cube, and fast states.
...
The agent uses 26 circle nodes plus two independent solid eye ellipses in a
100 × 100 logical field. Circles occupy a complete golden-angle Fibonacci
...
circle field, and pointer-down applies a wide radial impulse that reconverges
through a one-second inertial spring.

The state sequence alternates between the face and one randomly selected
globe, cube, or fast state. Fast mode runs the full eyed sphere at seven times
normal speed with a short persistence trail. Cube points use the same
golden-angle directions projected evenly across all six cube faces.
...
globe, cube, and fast controls directly select the current behavior. In Grid
mode the same controls navigate the four carousel entries; Auto resumes its
...
cues for the face, globe, cube, and fast states. No recorded audio files are
embedded. The speaker control disables all future cues.
